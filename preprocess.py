...
persons_num = 40
faces_num = 10
dataset_path = "D:\\k\\Myeigen\\MyEigenface\\att_faces"
...
if __name__ == "__main__":
    for i in range(1, persons_num + 1):
        ...
        for j in range(1, faces_num + 1):

            face_path = person_path + '/' + str(j) + '.pgm'
            eyes_path = person_path + '/' + str(j) + '.npy'
            
            print(i, j)
            point = np.load(eyes_path)
            x1 = int(point[0][0])
            y1 = int(point[0][1])
            x2 = int(point[1][0])
            y2 = int(point[1][1])
            img = Image.open(face_path)
            CropFace(img,eye_left=(x1, y1), eye_right=(x2, y2), offset_pct=(0.3, 0.3), dest_sz=(70, 70)).save(person_path + "/" + str(j) + '.png')
        print("finish a person!")